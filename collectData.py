...
import finnAPI
import readWrite
from utilities import safe_index # except errors when indexing (ie baz = json['key'])
...
def parseTenK(tenkJson):
    ...
    for tenK in filings:
        ...
        newRows.append([symbol, year, start, end, filed, netIncome,operating, investing, financing])
    return newRows
...
def collect_tenK_data(inputPath, outputPath):
    ...
    dataArr = []
    columns = ['symbol', 'year', 'start', 'end', 'filed', 'netIncome', 'operating', 'investing', 'financing']
...
def add_profile_data(inputPath, outputPath):
    ...
    df = df[df.netIncome.notnull() | df.operating.notnull() | df.investing.notnull() | df.financing.notnull()]
    symbol_dict = {}
    industry_arr = []
    marketCap_arr = []
