...
def analysis(df, endYear, startYear):
    print("\n###########################\nbeginning\n\n")
    df = df[df[naming_convention(endYear)].notnull() & df[naming_convention(startYear)].notnull()]
    df.info()
    horizontal = []
    horizontalPerc = []
    for index, row in df.iterrows():
        thisHorizontal = row[naming_convention(endYear)] - row[naming_convention(startYear)]
        thisHorizontalPerc = thisHorizontal * 100 / row[naming_convention(startYear)]
    ...
    df[analysis_naming_convention(endYear, startYear, percent=True)] = horizontalPerc
