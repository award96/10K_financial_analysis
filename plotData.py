...
def histo_by_percentile(filepath, column, percentile=(0,1), bins=20):
    # Validate params
    if type(percentile) != (tuple or list) or len(percentile) != 2 or percentile[0] < 0 or percentile[0] > 1 or percentile[1] < 0 or percentile[1] > 1 or percentile[0] > percentile[1]:
        raise ValueError(f"Percentile should be a tuple of (low_value, high_value) ranging from 0 to 1.\nThis was your input: {percentile}")
...
def histo_by_range(filepath, column, x_range=(float('-inf'),float('inf')), bins=20):
    # Validate params
    if type(x_range) != (tuple or list) or len(x_range) != 2 or x_range[0] > x_range[1]:
        raise ValueError(f"x_range should be a tuple or list of 2 values: (low_end, high_end)\nThis was your input: {x_range}")
...
def custom_histo(filepath, column, symbol=None, year=None, industry=None, title="", x_axis_label="", y_axis_label="", x_axis_scale=6):
    # filter by year
    # filter by industry
    # pick spec company
    # choose metric(s)
